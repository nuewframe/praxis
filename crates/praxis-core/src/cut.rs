...
fn release_kdl(
    ...
    // The cut composes this node whole, so a field the release already declared is lost
    // unless it is written here (`AS1`). What the record proposed and what the maintainer
    // confirmed are two facts, and a cut keeping only the second cannot be asked whether
    // they agreed.
