...
description: "Phase 5 of the principal-engineer workflow (implementer mode). Use after the capability layout and its decision are accepted, and the open iteration names the approach it follows, to write production-ready code with composition over inheritance, shift-left security, and structured telemetry. Implementer mode cannot modify accepted decisions, design specs, or a slice's cut scope — if a design flaw is discovered, bounce back to architect mode."
user-invocable: true
...
Project-specific guidance — exact token module paths, theme-object names, per-stack rules, and self-review greps — lives in the overlay `praxis adopt` writes into the host repository (`.github/instructions/capability-structure.instructions.md`). The plugin intentionally stays stack-agnostic; the overlay is where stack details belong, and it is a POINTER to this skill rather than a restatement of it.
