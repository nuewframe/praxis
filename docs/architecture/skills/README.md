...
| [ADR.260725.17: Context placement beyond one repository](../adr/ADR.260725.17-context-placement-beyond-one-repo.md) | Resolves multi-package and multi-repository context by extending the precedence stack with a package tier plus a `product_root` pointer, rather than generating per-package context. Retires the dual-home generator as a concept. **Status: Accepted** — implemented by `TS-002` of wave-brownfield-adoption. |
