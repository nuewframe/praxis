...
| [ADR.260724: Wave naming imposes no category taxonomy](../adr/ADR.260724-wave-category-relaxation.md) | Removes the `wave-feature-*` / `wave-platform-*` / `wave-ext-*` mandate from `create-wave` — the first shipped Praxis rule deleted by its own fourth scope-litmus question — and classifies rule *relaxation* as a governance case distinct from removal. |
| [ADR.260725.10: Retrofitting waves onto an existing product](../adr/ADR.260725.10-brownfield-wave-retrofit.md) | Adds a second wave form this pipeline did not have — the delivered wave, a README-only derived record whose slices cite evidence instead of declaring acceptance criteria — and the rule for choosing between it and the full four-document set. **Status: Accepted.** |
| [ADR.260725.17: Context placement beyond one repository](../adr/ADR.260725.17-context-placement-beyond-one-repo.md) | Resolves multi-package and multi-repository context by extending the precedence stack with a package tier plus a `product_root` pointer, rather than generating per-package context. Retires the dual-home generator as a concept. **Status: Proposed** — implementation is `TS-002` of wave-brownfield-adoption. |
